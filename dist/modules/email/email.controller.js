...
let EmailsController = class EmailsController {
    ...
    async sendEmailRecoveryPassword(body, request) {
        let code;
        code = Math.floor(Math.random() * 10000).toString();
        code = ('000' + code).slice(-4);
        return await this.emailSevices.sendEmailWithOTP('Recuperación de Contraseña', '../templates/recuperarClave', { Numero: code }, body, request);
    }
